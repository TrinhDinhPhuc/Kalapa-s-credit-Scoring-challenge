...
class eda_n_cleaning(object):
    ...
    def Modeling(self,data,label):
        self.data = data
        self.label = label
        self.data = self.data.drop(['FIELD_38','FIELD_18','FIELD_19','FIELD_20','FIELD_23','FIELD_25','FIELD_26','FIELD_27','FIELD_28'],axis=1)
        self.data = self.data.drop(['district'],axis=1)
        ...
        clf.fit(self.data_scaled, self.label)
        
        return clf
    def testing(self,model, test_data):
        self.model = model
        self.test_data = test_data
        self.y_predicted = model.predict(self.test_data)
        #from sklearn.metrics import roc_auc_score
        #roc_auc_score(test_data, self.y_predicted)
        return self.y_predicted

if __name__ == "__main__":
    obj_train = eda_n_cleaning("train.csv")
    data_train, data_train_label = obj_train.eda()
    #model = obj_train.Modeling(data_train, data_train_label)
    #obj_test = eda_n_cleaning("test.csv")
    #data_test, data_test_label = obj_test.eda()
    #obj_test.testing(model,data_test)
